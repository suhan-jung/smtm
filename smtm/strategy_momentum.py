...
class StrategyMomentum(Strategy):
    """
    가격 변화 Momentum을 활용한 매매 전략
    https://www.tradingview.com/support/solutions/43000599883-momentum-strategy/
    """
    ...
    def update_trading_info(self, info):
        ...
        self._update_momentum(info["closing_price"])
    ...
    def _update_position(self):
        ...
        """
        # self.logger.info(f"last MOM0 : {self.momentum_0[-1]}")
        # self.logger.info(f"last MOM0 : {self.momentum_0[-1]}")
        if len(self.momentum_1) == 0:
            self.position = None
        elif (self.momentum_0[-1] > 0) and (self.momentum_1[-1] > 0):
            self.position = "buy"
            self.logger.debug(f"[MOMENTUM] Update position to BUY {self.momentum_0[-1]}")
        elif (self.momentum_0[-1] < 0) and (self.momentum_1[-1] < 0):
            self.position = "sell"
            self.logger.debug(f"[MOMENTUM] Update position to SELL {self.momentum_0[-1]}")

    def _update_momentum(self, price):
        """전달 받은 종가 정보로 모멘텀 정보를 업데이트"""

        # data수가 momentum_length 보다 클때부터 시작
        if len(self.data) == self.MOMENTUM_LENGTH+1:
            momentum0 = price - self.data[-(self.MOMENTUM_LENGTH+1)]["closing_price"]
            self.momentum_0.append(momentum0)
            self.logger.debug(f"[MOMENTUM] Calculate MOM0 {momentum0}")
        elif len(self.data) > self.MOMENTUM_LENGTH+1:
            momentum0 = price - self.data[-(self.MOMENTUM_LENGTH+1)]["closing_price"]
            self.momentum_0.append(momentum0)
            momentum1 = self.momentum_0[-1] - self.momentum_0[-2]
            self.momentum_1.append(momentum1)
            self.logger.debug(f"[MOMENTUM] Calculate MOM0 {momentum0}")
            self.logger.debug(f"[MOMENTUM] Calculate MOM1 {momentum1}")
