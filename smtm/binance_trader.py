"""바이낸스 거래소를 통한 거래 요청 및 계좌 조회 요청을 처리하는 UpbitTrader 클래스"""
...
class BinanceTrader(Trader):
    ...
    AVAILABLE_CURRENCY = {"BTC": ("BTCUSDT", "BTC"), "ETH": ("ETHUSDT", "ETH"), "DOGE": ("DOGEUSDT", "DOGE"), "XRP": ("XRPUSDT", "XRP")}
    NAME = "Binance"

    def __init__(self, budget=50000, currency="BTCUSDT", commission_ratio=0.0005, opt_mode=True):
        if currency not in self.AVAILABLE_CURRENCY:
            raise UserWarning(f"not supported currency: {currency}")

        self.logger = LogManager.get_logger(__class__.__name__)
        self.worker = Worker("BinanceTrader-Worker")
        self.worker.start()
        self.timer = None
        self.order_map = {}
        self.ACCESS_KEY = os.environ.get("BINANCE_TEST_OPEN_API_ACCESS_KEY", "binance_access_key")
        self.SECRET_KEY = os.environ.get("BINANCE_TEST_OPEN_API_SECRET_KEY", "binance_secret_key")
        self.SERVER_URL = os.environ.get("BINANCE_TEST_OPEN_API_SERVER_URL", "binance_server_url")
    ...
    def _create_limit_order_query(market, is_buy, price, volume):
        query = {
            "symbol": market,
            "side": "BUY" if is_buy is True else "SELL",
            "type": "LIMIT",
            "timeInForce": "GTC",
            "quantity": str(volume),
            "price": str(price),
